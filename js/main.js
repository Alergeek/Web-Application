$(document).ready(function(){
	...
	var authToken = "12312312312312312312";
	var currentUser = new User("test@example.com", 2, authToken, []);
	...
	$('#button_profile').click(function(e){
		...
		var device;
		currentUser.getDevices(authToken)
		.then(function(devices){
				...
				for(device=0; device<devices.length; device++){
					if((devices[device].until*1000) > now.getTime()){
						if(devices[device].authToken!=authToken){
							...
							$('#'+devices[device].authToken).click(function(e){
								...
								currentUser.deleteDevice(this.id);
	...
	$('#button_new_device').click(function(e){
		...
		currentUser.getDevices(authToken).then(function(devices){
		...
		if($('.div_new_device').css('display')=='none'){
			...
			currentUser.getBarcode(authToken)
	...
	$('#save_profile').click(function(e){
		...
		if(	email=="" && new_pw=="" && confirm=="" ){
			...
			if(email != ""){
				currentUser.changeEmail(authToken, email, old_pw).then(function(result) {
					console.log(result);
					if(result == true){
			...
			if(new_pw!=""){
				if(new_pw!=confirm){
					...
					currentUser.changePassword(authToken, old_pw, new_pw).then(function(result) {
						console.log(result);
						if(result == true){
							displayAlert('Daten wurden erfolgreich geändert!','success');
						}else{
